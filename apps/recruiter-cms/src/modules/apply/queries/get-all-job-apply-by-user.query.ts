...
export class GetAllJobApplyByUserQueryHandler implements IQueryHandler<GetAllJobApplyByUserQuery> {
  constructor(private readonly applyRepository:ApplyRepository) {}

  async execute(query: GetAllJobApplyByUserQuery) {
    const { userId } = query;
    const jobsApply = await this.applyRepository.find({relations: ['job'],where: { userId }});
    return jobsApply;
  }
}