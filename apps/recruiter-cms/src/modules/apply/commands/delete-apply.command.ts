...
export class DeleteApplyCommandHandler implements ICommandHandler<DeleteApplyCommand> {
  ...
  async execute(command: DeleteApplyCommand) {
    ...
    const apply = await this.queryBus.execute(new GetOneApplyQuery(userId,jobID));
