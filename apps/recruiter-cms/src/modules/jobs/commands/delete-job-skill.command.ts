...
export class DeleteJobSkillCommandHandler implements ICommandHandler<DeleteJobSkillCommand> {
  ...
  async execute(command: DeleteJobSkillCommand): Promise<any> {
    const { id } = command;
    const query = this.jobSkillRepository.createQueryBuilder().delete().where('jobId IN (:id)',{id});
